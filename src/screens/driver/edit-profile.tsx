        ...
        uploadImage(image.path).then(url => {
          if (url && selectedImageCatgeory) {
            setvalues(prevValues => ({
              ...prevValues,
              [selectedImageCatgeory]: [...(prevValues.vehicleDocuments || []), url].filter(Boolean) // Ensuring all values are strings
            }));
      ...
      <Box sx={styles.main}>
        ...
        <Box sx={styles.genderContainer}>
          ...
          <Select
            ...
            onValueChange={(itemValue) => handleGenderChange(itemValue, 'gender')}
          >
        ...
        <Box sx={styles.genderContainer}>
          ...
          <Select
            selectedValue={values.gender}
            onValueChange={(itemValue) => handleGenderChange(itemValue, 'VehicleType')}
          >
            <SelectTrigger variant="outline" size="md">
              <SelectInput placeholder="Select Gender" defaultValue={values.VehicleType} />
            </SelectTrigger>
            <SelectPortal>
              <SelectBackdrop />
              <SelectContent>
        ...
        <Box sx={styles.inputBox} >
          ...
          <Box sx={styles.pictureBox} >
            {values?.vehicleDocuments && values?.vehicleDocuments?.length > 0 &&
              values?.vehicleDocuments.map((data, index) => {
                return (
                  <Box key={index} sx={styles.imageRelative} >
                    <MaterialIcons onPress={() => handleRemoveImage('vehicleDocuments', index)} name='cancel' style={styles.cancel} />
                    <Image alt='ops' sx={styles.img} source={{ uri: data }} />
                  </Box>
                )
              })
            }
            {values?.vehicleDocuments && values?.vehicleDocuments?.length < 4 &&
              (<Pressable onPress={() => {
                setModalVisible(true);
                setselectedImageCatgeory('vehicleDocuments')
              }} style={{ height: 80, width: 80, borderRadius: 10, alignItems: "center", justifyContent: "center", backgroundColor: colors.grey, opacity: 0.8 }} >
          ...
          <Box sx={styles.pictureBox} >
            {values?.license && values?.license?.length > 0 &&
              values?.license.map((data, index) => {
                return (
                  <Box key={index} sx={styles.imageRelative} >
                    <MaterialIcons onPress={() => handleRemoveImage('license', index)} name='cancel' style={styles.cancel} />
                    <Image alt='ops' sx={styles.img} source={{ uri: data }} />
                  </Box>
                )
              })
        ...
        <Box sx={styles.inputBox} >
          ...
          <Box sx={styles.pictureBox} >
            {values?.vehicleImages && values?.vehicleImages?.length > 0 &&
              values?.vehicleImages.map((data, index) => {
                return (
                  <Box key={index} sx={styles.imageRelative} >
                    <MaterialIcons onPress={() => handleRemoveImage('vehicleImages', index)} name='cancel' style={styles.cancel} />
                    <Image alt='ops' sx={styles.img} source={{ uri: data }} />
                  </Box>
                )
              })
